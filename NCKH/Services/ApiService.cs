...
namespace NCKH.Services
{
    ...
    public class ApiService
    {
        ...
        public async Task<string> AddHoSoAsync(HoSo hoso)
        {
            try
            {
                using var content = new MultipartFormDataContent();

                var titleContent = new StringContent(hoso.TieuDe ?? "");
                titleContent.Headers.ContentType = null;
                content.Add(titleContent, "title");

                var descContent = new StringContent(hoso.MoTa ?? "");
                descContent.Headers.ContentType = null;
                content.Add(descContent, "description");

                var response = await _httpClient.PostAsync(
                    $"{BaseUrl}/api/Applications/CreateApplication", content);

                var responseBody = await response.Content.ReadAsStringAsync();
                System.Diagnostics.Debug.WriteLine($"[API_CREATE] Status: {response.StatusCode}, Body: {responseBody}");
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[API_CREATE] Exception: {ex.Message}");
            }
            return null;
        }
        ...
        public async Task<bool> UpdateHoSoAsync(string id, HoSo hoso)
        {
            try
            {
                ...
                System.Diagnostics.Debug.WriteLine($"[UPDATE] StatusCode: {response.StatusCode}, Body: {body}");
        ...
        public async Task<bool> UploadFileAsync(string hoSoId, FileResult file)
        {
            try
            {
                ...
                // applicationId
                var idContent = new StringContent(hoSoId);
                idContent.Headers.ContentType = null;
                formData.Add(idContent, "applicationId");
                ...
                var response = await _httpClient.PostAsync(
                    $"{BaseUrl}/api/ApplicationFiles/CreateApplicationFiles", formData);

                var body = await response.Content.ReadAsStringAsync();
                System.Diagnostics.Debug.WriteLine($"[UPLOAD] Status: {response.StatusCode}, Body: {body}");
                return response.IsSuccessStatusCode;
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> DeleteFileAsync(string hoSoId, string fileId)
        {
            ...
            {
                var response = await _httpClient.DeleteAsync(
                    $"{BaseUrl}/api/ApplicationFiles/{hoSoId}/{fileId}");
